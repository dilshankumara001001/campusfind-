...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title) || empty($category) || empty($location) || empty($date_found)) {
        ...
        try {
            $stmt = $pdo->prepare("UPDATE found_items SET category=?, title=?, description=?, location=?, date_found=? WHERE id=? AND user_id=?");
            $stmt->execute([$category, $title, $description, $location, $date_found, $id, $user_id]);
            logActivity($user_id, 'edit_found', "Edited found item: $title");
...
<head>
    ...
    <title>Edit Found Item - <?= SITE_NAME ?></title>
            ...
            <div class="col-lg-8">
                <div class="card">
                    <div class="card-header">
                        <h4 class="mb-0">✏️ Edit Found Item</h4>
                    </div>
                    <div class="card-body">
                        <?php if ($error): ?>
                        ...
                        <form method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="date" class="form-control" name="date_found" value="<?= $item['date_found'] ?>" required>
                            </div>
                            <button type="submit" class="btn btn-primary">Update Item</button>
                            <a href="found-items.php" class="btn btn-secondary">Cancel</a>
                        </form>
                    </div>
                </div>
            </div>
